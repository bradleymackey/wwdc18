...
public final class CageSprite: SKSpriteNode {
	...
	private func physicsBody(texture:SKTexture) -> SKPhysicsBody {
		...
		let body = SKPhysicsBody(circleOfRadius: self.spriteSize.width/2)
		body.categoryBitMask = PhysicsCategory.chainLink
		body.affectedByGravity = true
		body.collisionBitMask = PhysicsCategory.all ^ (PhysicsCategory.box) // collide with all but box and keys
